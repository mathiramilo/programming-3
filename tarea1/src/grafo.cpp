...
static Vertice copia_vertice(Vertice v) {
  ...
  res->incidentes = copia_lista(v->incidentes);
...
void remover_vertice(nat v, Grafo &g) {
  ...
  if (g->celdas[v].existe) {
    ...
    nat m = g->dirigido? g->celdas[v].vertice->outgrado + g->celdas[v].vertice->ingrado : g->celdas[v].vertice->outgrado;
    g->m -= m;
    Lista lst = g->celdas[v].vertice->adyacentes;
    comienzo(lst);
    while (existe_actual(lst)) {
      nat w = actual(lst);
      g->celdas[w].vertice->ingrado--;
      remover(v, g->celdas[w].vertice->incidentes);
      siguiente(lst);
    }
    lst = g->celdas[v].vertice->incidentes;
    comienzo(lst);
    while (existe_actual(lst)) {
      nat w = actual(lst);
      g->celdas[w].vertice->outgrado--;
      remover(v, g->celdas[w].vertice->adyacentes);
      siguiente(lst);
    }
    destruir_vertice(g->celdas[v].vertice);
...
void remover_arista(nat v1, nat v2, Grafo g) {
  ...
  if (pertenece_a_lista(v2, g->celdas[v1].vertice->adyacentes)) {
    ...
    remover(v1, g->celdas[v2].vertice->incidentes);
    if (!g->dirigido) {
      assert(pertenece_a_lista(v1, g->celdas[v2].vertice->adyacentes));
      g->celdas[v2].vertice->outgrado--;
      g->celdas[v1].vertice->ingrado--;
      remover(v1, g->celdas[v2].vertice->adyacentes);
      remover(v2, g->celdas[v1].vertice->incidentes);
      assert(g->celdas[v1].vertice->outgrado == g->celdas[v1].vertice->ingrado);
      assert(g->celdas[v2].vertice->outgrado == g->celdas[v2].vertice->ingrado);
    }
  }
...
Grafo traspuesto(Grafo g) {
  ...
  for (nat i = 1; i <= g->max; i++)
    if (existe_vertice(i, g)) {
      ...
      comienzo(lst);
      while (existe_actual(lst)) {
        assert(existe_vertice(actual(lst), t));
        insertar_al_final(i, t->celdas[actual(lst)].vertice->adyacentes);
        insertar_al_final(actual(lst), t->celdas[i].vertice->incidentes);
        siguiente(lst);
      }
    }
  return t;
...
Grafo inducido(Lista vertices, Grafo g) {
  ...
  comienzo(lst);
  while (existe_actual(lst)) {
    assert(existe_vertice(actual(lst), g));
    agregar_vertice(actual(lst), res);
    siguiente(lst);
  }
  ...
  while (existe_actual(lst)) {
    ...
    while (existe_actual(ady)) {
      if (existe_vertice(actual(ady), res)) {
        res->m++;
        res->celdas[v].vertice->outgrado++;
        res->celdas[actual(ady)].vertice->ingrado++;
        insertar_al_final(actual(ady), res->celdas[v].vertice->adyacentes);
        insertar_al_final(v, res->celdas[actual(ady)].vertice->incidentes);
      }
      siguiente(ady);
    }
    siguiente(lst);
...
bool es_ciclo(Lista vertices, Grafo g) {
  ...
  comienzo(lst);
  nat v, v_prim;
  if (!existe_actual(lst) || cantidad_elementos(lst) < 3) {
    return false;
  } else {
    v = v_prim = actual(lst);
    bool* visitados = new bool[g->n + 1];
    memset(visitados, 0, (g->n + 1) * sizeof(bool)); //Inicializamos las variables en false
    visitados[v] = true;
    siguiente(lst);
    while (existe_actual(lst) && !visitados[actual(lst)] && existe_arista(v, actual(lst), g)) {
      v = actual(lst);
      visitados[v] = true;
      siguiente(lst);
    }
    delete [] visitados;
    if (existe_actual(lst) && actual(lst) == v_prim && existe_arista(v, v_prim, g)) {
      siguiente(lst);
      return !existe_actual(lst);
    } else 
      return false;
  }
}
